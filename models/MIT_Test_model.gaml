...
global {
	...
	file shape_file_roads <-  file("../includes/shp/events/roads_mit.shp");
	file shape_file_nodes <- file("../includes/shp/events/nodes_mit.shp");
	...
	// this column to the attribute is_traffic_signal. 
		...
		create road from: shape_file_roads with: [num_lanes::int(read("num_lanes")), oneway::string(read("oneway"))] {
...
experiment experiment_city type: gui {
	...
	action _init_{
		create simulation with:[
			shape_file_roads::file("../includes/shp/events/roads_mit.shp"), 
			shape_file_nodes::file("../includes/shp/events/nodes_mit.shp"),
			nb_people::200
		];
	}
	output {
