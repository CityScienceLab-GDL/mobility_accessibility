...
species project{
	...
	action create_new_activities{
		...
		loop i over: sample(allowed_activities_for_me,nb_ec_units,true){
			create economic_unit with:[from_scenario::"B",sub_id::i] returns:new_unit;
	...
	action deactivate{
		ask people where(each.from_scenario="B"){do die;}
		ask economic_unit where(each.from_scenario="B"){do die;}
	}
	aspect default{
